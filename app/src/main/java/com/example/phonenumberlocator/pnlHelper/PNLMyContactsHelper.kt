...
import com.example.phonenumberlocator.pnlHelper.contactBackgroundImagesIds
import com.example.phonenumberlocator.pnlHelper.ensureBackgroundThread
import com.example.phonenumberlocator.pnlHelper.getContrastColor
import com.example.phonenumberlocator.pnlHelper.getQuestionMarks
import com.example.phonenumberlocator.pnlHelper.letterBackgroundColors
...
import com.example.phonenumberlocator.pnlExtensionFun.getNameLetter
...
class PNLMyContactsHelper(val context: Context) {
    ...
    fun getAvailableContacts(favoritesOnly: Boolean, callback: (ArrayList<PNLMyContact>) -> Unit) {
        ensureBackgroundThread {
            ...
            }.distinctBy { it.rawId }.toMutableList() as ArrayList<PNLMyContact>
            ...
            val contactsToRemove = ArrayList<PNLMyContact>()
            allContacts.groupBy { it.name }.forEach {
                val contacts = it.value.toMutableList() as ArrayList<PNLMyContact>
                if (contacts.size > 1) {
    ...
    private fun getContactNames(favoritesOnly: Boolean): List<PNLMyContact> {
        ...
        context.queryCursor(uri, projection, selection, selectionArgs) { cursor ->
            ...
            if (isPerson) {
                ...
                if (firstName.isNotEmpty() || middleName.isNotEmpty() || familyName.isNotEmpty()) {
                    ...
                    val contact = PNLMyContact(rawId,
                        contactId,
                        fullName,
                        photoUri,
                        ArrayList(),
    ...
    private fun getContactPhoneNumbers(favoritesOnly: Boolean): ArrayList<PNLMyContact> {
        val contacts = ArrayList<PNLMyContact>()
        ...
        context.queryCursor(uri, projection, selection) { cursor ->
            ...
            if (contacts.firstOrNull { it.rawId == rawId } == null) {
                val contact =
                    PNLMyContact(rawId, contactId, "", "", ArrayList(), ArrayList(), ArrayList())
                contacts.add(contact)
            }

            val phoneNumber =
                PNLLocatorPhoneNumber(normalizedNumber, type, label, normalizedNumber, isPrimary)
    ...
    fun loadContactImage(
        ...
        val placeholder = placeholderImage ?: BitmapDrawable(context.resources,
    ...
    fun loadContactImageWithTextColor(imageView: ImageView, placeholderName: String, ) {
        val imageId = getContactCustomIconID(placeholderName)
    ...
    private fun getContactCustomIconID(name: String): Int {
        val drawable_id =
            contactBackgroundImagesIds[abs(name.hashCode()) % contactBackgroundImagesIds.size]
        return drawable_id
    }
...
*/
    ...
    fun exists(number: String, privateCursor: Cursor?, callback: (Boolean) -> Unit) {
        PNLMyContactsHelper(context).getAvailableContacts(false) { contacts ->
            val contact = contacts.firstOrNull { it.doesHavePhoneNumber(number) }
            if (contact != null) {
                callback.invoke(true)
            } else {
                val privateContacts = PNLMyContactProvider.getSimpleContacts(privateCursor)
    ...
    fun deleteContact(
        originalContact: PNLMyContact,
        callback: (success: Boolean) -> Unit,
    ) {
        ensureBackgroundThread {
            if (deleteContacts(originalContact)) {
                callback(true)
            }
        }
    }
    ...
    fun deleteContacts(contact: PNLMyContact): Boolean {
        return try {
            ...
            ContentProviderOperation.newDelete(ContactsContract.RawContacts.CONTENT_URI).apply {
                val selectionArgs = arrayOf(contact.rawId.toString())
    ...
    fun getContactLookupKey(contactRawId: String): String {
        val uri = Data.CONTENT_URI
        val projection = arrayOf(Data.CONTACT_ID, Data.LOOKUP_KEY)
        val selection = "${Data.MIMETYPE} = ? AND ${Data.RAW_CONTACT_ID} = ?"
        val selectionArgs = arrayOf(StructuredName.CONTENT_ITEM_TYPE, contactRawId)
    ...
    fun getLookupKey(contactId: String): String? {
        var lookupKey: String? = null
        val projection = arrayOf(ContactsContract.Contacts.LOOKUP_KEY)
        val selection: String = ContactsContract.Contacts._ID + "=" + contactId
        val c: Cursor? = context.contentResolver.query(ContactsContract.Contacts.CONTENT_URI, projection, selection, null, null)
        if (c != null) {
            if (c.moveToFirst()) {
